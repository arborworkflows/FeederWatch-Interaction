...
/** 
 * Added to allow delete of rows by selecting the delete cell (and invoking this callback)
 * this callback is called automatically every time an editable cell in the grid is changed.
 */
...
EditableGrid.prototype.getRowIndex = function(rowId)
{
	...
	// CL - ***kludge *** seemed like an index error here, first and second row acted the same, changed structure component 
	// being compared to data[rowIndex].originalIndex.  Works now on small testcases.  This might not work on filtered data, but
	// we aren't using the filter on this table instance, in this application. 

	//for (var rowIndex = 0; rowIndex < this.data.length; rowIndex++) if (this.data[rowIndex].id == rowId) return rowIndex;
	for (var rowIndex = 0; rowIndex < this.data.length; rowIndex++) if (this.data[rowIndex].originalIndex == rowId) return rowIndex;
	return -1;
